/**
 * UNIT_TEMPLATES — all 44 player-trainable unit progression templates.
 * Each template can be used as both a training target and an army category.
 *
 * buildingRequirements: [{ type: 'building_type', minLevel: N }]
 *   A template is unlocked when the fief has ALL required buildings completed at the specified level.
 *
 * baseDays / baseCost: cost per unit trained (already incorporates tier multiplier).
 * Military stat further reduces training days: each point above 1 = -4%, max -36% at stat 10.
 */
...
const UNIT_TEMPLATES = {
  ...
  'Longbowman': {
    ...
    stats: { equipment: 6, discipline: 10, morale: 8, command: 6, logistics: 7 },
  },
  'Crossbowman': {
    tier: 3, baseType: 'archer', icon: '🎯',
    ...
    baseDays: 20, baseCost: { gold: 32, food: 12 },
    stats: { equipment: 6, discipline: 4, morale: 5, command: 5, logistics: 6 },
  },
  'Arbalest': {
    tier: 4, baseType: 'archer', icon: '🎯',
    ...
    baseDays: 40, baseCost: { gold: 64, food: 24 },
    stats: { equipment: 8, discipline: 6, morale: 6, command: 6, logistics: 7 },
  },
  'Mounted Archer': {
    tier: 3, baseType: 'archer', icon: '🏇',
    parents: ['Ranger', 'Man-at-Arms'], children: ['Horse Archer'],
    // Reachable from either archer T2 or cavalry T2
  ...
  'Knight': {
    ...
    stats: { equipment: 9, discipline: 10, morale: 7, command: 6, logistics: 5 },
  },
  'Lancer': {
...
/**
 * Check whether a template is unlocked for a given set of completed fief buildings.
 * @param {string} templateName
 * @param {Array<{building_type: string, level: number}>} completedBuildings
 */
function isTemplateUnlocked(templateName, completedBuildings) {
...
function getUnlockedTemplates(completedBuildings) {
  return Object.keys(UNIT_TEMPLATES).filter(name => isTemplateUnlocked(name, completedBuildings));
}

/** All valid army category names (used for DB constraint). */
const ALL_CATEGORY_NAMES = Object.keys(UNIT_TEMPLATES);

module.exports = { UNIT_TEMPLATES, isTemplateUnlocked, getUnlockedTemplates, ALL_CATEGORY_NAMES };
